    ...
    <section id="contas-page" class="mt-6">
        ...
        <div class="flex items-center justify-between mb-4">
            <div>
                ...
                <p class="text-sm text-neutral-500 dark:text-neutral-400">Acompanhe saldos por banco e acesse o extrato de cada conta.</p>
            </div>
        ...
        <!-- Modal Conta -->
        <div id="accModal" class="fixed inset-0 z-[60] hidden" role="dialog" aria-modal="true" aria-labelledby="accModalTitle">
            ...
            <div class="absolute inset-x-0 bottom-0 md:inset-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:w-[560px]">
                <div class="rounded-t-3xl md:rounded-2xl border border-neutral-200/70 dark:border-neutral-800/70 bg-white dark:bg-neutral-900 shadow-soft dark:shadow-softDark p-4 md:p-6">
                    <div class="flex items-start justify-between">
                        <div>
                            <h3 id="accModalTitle" class="text-lg font-semibold" data-crud-title>Nova conta bancária</h3>
                    ...
                    <div id="accFormErr" class="hidden mb-2 rounded-lg bg-red-50 text-red-700 text-sm px-3 py-2" data-form-error></div>
                    ...
                    <form id="accForm" class="mt-4 grid gap-3" novalidate>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <label class="block">
                                ...
                                <input id="current_balance" name="current_balance" inputmode="decimal" placeholder="0,00"
                                       class="mt-1 w-full rounded-xl border border-neutral-200/70 dark:border-neutral-800/70 bg-white/90 dark:bg-neutral-900/70 px-3 py-2" required/>
                                <p class="field-error mt-1 text-xs text-red-600 hidden"></p>
                            </label>
                            ...
                            <label class="block">
                                ...
                                <div class="mt-1 inline-flex w-full rounded-xl border border-neutral-200/70 dark:border-neutral-800/70 bg-neutral-50 dark:bg-neutral-800 p-1">
                                    <input type="radio" name="type" value="1" id="accCorr" class="peer/acc1 hidden" checked>
                                    <label for="accCorr" class="flex-1 text-center px-3 py-1.5 rounded-lg bg-white dark:bg-neutral-900 shadow-sm cursor-pointer peer-checked/acc1:font-medium">Corrente</label>

                                    <input type="radio" name="type" value="2" id="accPoup" class="peer/acc2 hidden">
                                    <label for="accPoup" class="flex-1 text-center px-3 py-1.5 rounded-lg cursor-pointer hover:bg-white/70 dark:hover:bg-neutral-900/70">Poupança</label>

                                    <input type="radio" name="type" value="3" id="accInv" class="peer/acc3 hidden">
                                    <label for="accInv" class="flex-1 text-center px-3 py-1.5 rounded-lg cursor-pointer hover:bg-white/70 dark:hover:bg-neutral-900/70">Investimento</label>
                                </div>
                                <p class="field-error mt-1 text-xs text-red-600 hidden"></p>
                            </label>
                        </div>
                        ...
                        <div class="mt-2 flex items-center justify-end gap-2">
                            <button type="button" id="accCancel" data-crud-cancel
                                    class="px-3 py-2 rounded-xl border border-neutral-200/70 dark:border-neutral-800/70 hover:bg-neutral-50 dark:hover:bg-neutral-800">Cancelar</button>
                ...
                <div class="rounded-t-3xl md:rounded-2xl border border-neutral-200/70 dark:border-neutral-800/70 bg-white dark:bg-neutral-900 shadow-soft dark:shadow-softDark p-4 md:p-6">
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <p class="text-sm text-neutral-500 dark:text-neutral-400">Selecione a conta de destino e o valor.</p>
                        </div>
                        <button id="trClose" class="size-10 grid place-items-center rounded-xl hover:bg-neutral-100 dark:hover:bg-neutral-800" aria-label="Fechar">
                            <svg class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18"/><path d="M6 6l12 12"/></svg>
                        </button>
                    </div>
                    ...
                    <form id="trForm" class="mt-4 grid gap-3" novalidate>
                        ...
                        <label class="block">
                            ...
                            <select id="trTo" name="to_id" class="mt-1 w-full rounded-xl border border-neutral-200/70 dark:border-neutral-800/70 bg-white/90 dark:bg-neutral-900/70 px-3 py-2" required></select>
                        ...
                        <label class="block">
                            ...
                            <input id="trAmount" name="amount" inputmode="decimal" placeholder="0,00" class="mt-1 w-full rounded-xl border border-neutral-200/70 dark:border-neutral-800/70 bg-white/90 dark:bg-neutral-900/70 px-3 py-2" required/>
                            <p id="trAmountErr" class="mt-1 text-xs text-red-600 hidden"></p>
                        </label>

                        <div class="mt-2 flex items-center justify-end gap-2">
                            <button type="button" id="trCancel" class="px-3 py-2 rounded-xl border border-neutral-200/70 dark:border-neutral-800/70 hover:bg-neutral-50 dark:hover:bg-neutral-800">Cancelar</button>
                            <button type="submit" class="px-4 py-2 rounded-xl bg-brand-600 hover:bg-brand-700 text-white shadow-soft">Transferir</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div id="accSheet" class="fixed inset-0 z-[70] hidden" aria-modal="true" role="dialog">
            ...
            <div class="absolute inset-x-0 bottom-0 rounded-t-2xl border border-neutral-200/60 dark:border-neutral-800/60 bg-white dark:bg-neutral-900 shadow-soft p-2">
                ...
                <div class="grid gap-1 p-1">
                    ...
                    <button data-sheet-action="transfer" class="w-full text-left px-4 py-3 rounded-xl hover:bg-neutral-50 dark:hover:bg-neutral-800">Transferir</button>
                    <button data-sheet-action="statement" class="w-full text-left px-4 py-3 rounded-xl hover:bg-neutral-50 dark:hover:bg-neutral-800">Ver extrato</button>
    ...
    @push('scripts')
        <script>
            (() => {
                ...
                const readCache = () => { try{ return JSON.parse(localStorage.getItem(ACC_CACHE_KEY)) || null; }catch{ return null; } };
                const writeCache = (accounts, savings) => { try{ localStorage.setItem(ACC_CACHE_KEY, JSON.stringify({accounts,savings,t:Date.now()})); }catch{} };
    ...
    <div class="rounded-xl border border-neutral-200/70 dark:border-neutral-800/70 p-3">
      ...
      <p class="text-lg font-medium" data-cofr>${brl(cofr)}</p>
                ...
                grid.addEventListener('click', async (e)=>{
                    ...
                    if (btn){
                        e.preventDefault(); suppressUntil = Date.now()+400;
                        const act = btn.dataset.action;

                        if (act==='transfer'){ openTransfer(id); return; }
                        if (act==='edit'){
                            try{
                                const res = await fetch(u(ROUTES.show,id), {headers:{'Accept':'application/json','X-Requested-With':'XMLHttpRequest'}});
                                if(!res.ok) throw 0;
                                const acc = await res.json();
                                currentId = id; openModal('edit', acc);
                        ...
                        if (act==='more'){ openSheet(id); return; }
                        return;
                    }
                    ...
                    try{
                        ...
                        if(!res.ok) throw 0;
                        const acc = await res.json();
                        currentId = id; openModal('show', acc);
                ...
                sheet.addEventListener('click', async (e)=>{
                    ...
                    if (act==='transfer'){ closeSheet(); openTransfer(sheetId); return; }
                    if (act==='statement'){ closeSheet(); window.location.href = ROUTES.tx + '?account=' + encodeURIComponent(sheetId); return; }
                    if (act==='edit'){
                        try{
                            const res = await fetch(u(ROUTES.show,sheetId), {headers:{'Accept':'application/json','X-Requested-With':'XMLHttpRequest'}});
                            if(!res.ok) throw 0;
                            const acc = await res.json();
                            closeSheet(); currentId = sheetId; openModal('edit', acc);
                        }catch{ alert('Erro ao carregar conta'); }
                        return;
                    }
                    if (act==='delete'){
                        const id = sheetId; closeSheet();
                        if (!confirm('Excluir esta conta?')) return;
                        try{ await doDeleteAccount(id); const el=[...grid.querySelectorAll('article[data-id]')].find(n=>n.dataset.id==id); el?.remove(); }
                        catch{ alert('Erro ao excluir'); }
                        return;
                    }
                });
                ...
                function openTransfer(fromId){
                    clearTransferErrors();
                    trForm.reset(); trFrom.value = fromId;
                ...
                function closeTransfer(){ trModal.classList.add('hidden'); document.body.classList.remove('overflow-hidden','ui-modal-open'); }
                ...
                function clearFieldError(inputEl, id){
                    const el = document.getElementById(id); el?.classList.add('hidden'); if (el) el.textContent='';
                    inputEl?.classList.remove('ring-2','ring-red-500/40','border-red-500');
                }
                function showFieldError(inputEl, id, msg){
                    const el = document.getElementById(id); if (el){ el.textContent = msg || 'Campo inválido'; el.classList.remove('hidden'); }
                    inputEl?.classList.add('ring-2','ring-red-500/40','border-red-500');
                }
                function clearTransferErrors(){
                    clearFieldError(trTo,'trToErr'); clearFieldError(trAmount,'trAmountErr');
                    const g = document.getElementById('formError'); if (g){ g.classList.add('hidden'); g.textContent=''; }
                }
                function showTransferError(msg){
                    const g = document.getElementById('formError'); if (g){ g.textContent = msg || 'Erro ao enviar'; g.classList.remove('hidden'); }
                }
                ...
                trForm.addEventListener('submit', async (e)=>{
                    e.preventDefault(); clearTransferErrors();
